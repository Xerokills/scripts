...
mouse.KeyDown:connect(function(key)
    ...
    elseif key == 'u' then
        local status = is_on()
        starterGui:SetCore('SendNotification', {Title = "Dmg Amp Hitware", Text = 'Hitware is '..status..'. Current: '.. distance..' studs'})
    elseif key == "q" then
        distance = distance + 1
        starterGui:SetCore("SendNotification", {Title = "Dmg Amp Hitware", Text = "Hitware has increased by 1. Current: " .. distance..' studs'})
    elseif key == "e" then
        if distance > 1 then
            distance = distance - 1
            starterGui:SetCore("SendNotification", {Title = "Dmg Amp Hitware", Text = "Hitware has decreased by 1. Current: " .. distance..' studs'})
        else
            starterGui:SetCore("SendNotification", {Title = "Dmg Amp Hitware", Text = "Cannot decrease any further!"})
        end
    end
end)
